...
const NewPancardScreen = ({route, navigation}) => {
  const {passportData} = route.params; // Get passportData from route params
  const [pancardNumber, setPancardNumber] = useState('');
  const [pancardPhotoUrl, setPancardPhotoUrl] = useState(''); // State for Pancard photo URL
  ...
  const handleNext = () => {
    ...
    navigation.navigate('TravellerPhotoScreen', {passportData: updatedPassportData});
  };
