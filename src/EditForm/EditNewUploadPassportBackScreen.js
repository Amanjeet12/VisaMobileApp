...
const EditNewUploadPassportBackScreen = ({route, navigation}) => {
  const {passportData} = route.params; // Get passportData from route params
  console.log(passportData)

  // Initialize state with pre-filled values if available
  const [fatherName, setFatherName] = useState(passportData.fatherName || '');
  const [motherName, setMotherName] = useState(passportData.motherName || '');
  const [passportBackUrl, setPassportBackUrl] = useState(passportData.passport_back || ''); // State to store the passport back URL
  ...
  const handleNext = () => {
    ...
    navigation.navigate('EditNewPancad', {passportData: updatedPassportData});
  };
      ...
      <ScrollView
        ...
        <View>
          <ImageBox
            ...
            initialImageUrl={passportBackUrl} // Pass initial image URL
...
export default EditNewUploadPassportBackScreen;
